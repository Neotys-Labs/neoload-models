...
public class ThinktimeWriterTest {
    ...
    public void writeThinktimeXmlTest() throws ParserConfigurationException {
    	...
    	final ThinkTimeConstant thinktime = new ThinkTimeConstant.Builder().name("myThinktime").value("1000").description("myDescription").build();
    	final String expectedResult = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>"
    			...
    			+ "</test-root>";	
    	ThinkTimeConstantWriter.of(thinktime).writeXML(doc, root, Files.createTempDir().getAbsolutePath());
