...
public class DelayWriterTest {
    ...
    public void writeDelayXmlTest() throws ParserConfigurationException {
    	...
    	final DelayConstant delay = new DelayConstant.Builder().name("myDelay").value("1000").description("myDescription").build();
    	final String expectedResult = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>"
    			...
    			+ "</test-root>";	
    	DelayConstantWriter.of(delay).writeXML(doc, root, Files.createTempDir().getAbsolutePath());
