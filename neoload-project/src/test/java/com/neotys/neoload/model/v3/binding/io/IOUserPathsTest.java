...
public class IOUserPathsTest extends AbstractIOElementsTest {
	...
	// the new key, so the legacy-keyed fixture above (whose UserPath1 exercises the hand-written
	// UserPathSerializer/Deserializer fallback, not the generic @JsonAlias binding used by
	// Request/Transaction/Container/Case) can only be reused for read.
	@Test
	public void writeUserPathsRequiredAndOptional() throws IOException {
		final Project expectedProject = getUserPathsRequiredAndOptional();
		assertNotNull(expectedProject);

		write("test-userpaths-required-and-optional-content-assertions", expectedProject);
	}

	@Test
	public void readUserPathsRequiredAndOptionalContentAssertions() throws IOException {
		final Project expectedProject = getUserPathsRequiredAndOptional();
		assertNotNull(expectedProject);

		read("test-userpaths-required-and-optional-content-assertions", expectedProject);
	}
}
