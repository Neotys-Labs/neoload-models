...
public class UserPathDeserializerTest {
	...
	public void legacyAssertionsKeyFallsBackToContentAssertions() throws IOException {
		...
		final List<Assertion> expected = ImmutableList.of(ContentAssertion.builder().contains("legacy").build());
		assertEquals(expected, userPath.getAssertions());
		assertEquals(userPath.getAssertions(), userPath.getContentAssertions());
	}
	...
	public void newContentAssertionsKeyLeavesLegacyAssertionsEmpty() throws IOException {
		...
		final List<Assertion> expected = ImmutableList.of(ContentAssertion.builder().contains("new_key").build());
		assertEquals(expected, userPath.getContentAssertions());
		assertTrue(userPath.getAssertions().isEmpty());
	}
}
