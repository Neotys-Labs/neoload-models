...
import com.neotys.neoload.model.v3.validation.constraints.MutuallyExclusiveAssertionsFieldCheck;
...
/**
 * Validates whichever of {@code assertions} (legacy) or {@code content_assertions} is the
...
public final class AssertionsFieldValidator extends AbstractConstraintValidator<AssertionsFieldCheck, AssertionsElement> {
	...
	public boolean isValid(final AssertionsElement element, final ConstraintValidatorContext context) {
		...
		final List<ContentAssertion> legacyAssertions = element.getLegacyAssertions();
		final List<ContentAssertion> contentAssertions = element.getContentAssertions();
		final boolean explicitContentAssertions = legacyAssertions != contentAssertions;
		final String fieldName = explicitContentAssertions ? AssertionsElement.CONTENT_ASSERTIONS : AssertionsElement.ASSERTIONS;
		final List<ContentAssertion> effective = explicitContentAssertions ? contentAssertions : legacyAssertions;

		boolean valid = true;
		context.disableDefaultConstraintViolation();

		if (explicitContentAssertions && !legacyAssertions.isEmpty()) {
			context.buildConstraintViolationWithTemplate(messageTemplate(MutuallyExclusiveAssertionsFieldCheck.class))
					.addPropertyNode(AssertionsElement.CONTENT_ASSERTIONS)
		...
		for (int i = 0; i < effective.size(); i++) {
			final ContentAssertion contentAssertion = effective.get(i);
