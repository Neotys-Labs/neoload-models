...
public class SupportLevelCounterTest {
	...
	public void test() {
		...
		assertEquals("0%", counter.getTotalCoveragePercentAsString());
		counter.nextScript();
		...
		assertEquals("100%", counter.getTotalCoveragePercentAsString());
		...
		assertEquals("\tSupported: 1 (without warning) + 0 (with warning). \n\tUnsupported: 1\n			*1: titi\n", counter.getTotalSummary());
		assertEquals("50%", counter.getTotalCoveragePercentAsString());
